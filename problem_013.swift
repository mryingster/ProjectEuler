#!/usr/bin/xcrun swift
// -*- compile-command: "swiftc problem_013.swift -o problem_013" -*-
...
let numArray: [[Int]] = [[3,7,1,0,7,2,8,7,5,3,3,9,0,2,1,0,2,7,9,8,7,9,7,9,9,8,2,2,0,8,3,7,5,9,0,2,4,6,5,1,0,1,3,5,7,4,0,2,5,0],
...
var sumArray: [Int] = [Int](count: 100, repeatedValue: 0)

// Add all rows together into answer array by column
for var r = 0 ; r < numArray.count ; r++ {
    for var c = 0 ; c < numArray[0].count ; c++ {
        sumArray[c+(sumArray.count-numArray[0].count)] += numArray[r][c]
    }
}

// Carry over answers for each column
for var c = sumArray.count - 1; c > 0; c-- {
    while sumArray[c] > 9 {
        sumArray[c-1]++
        sumArray[c] -= 10
    }
...
var pos = 0
var len = 10
while pos < sumArray.count && len > 0 {
    pos++
    if sumArray[pos] == 0 && len == 10 {
        continue
    }
    print(sumArray[pos], terminator: "")
    len--
}
print("")
